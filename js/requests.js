...
async function initRequestForm() {
  ...
  // Load payment numbers from settings
  loadPaymentNumbers();
}
...
function goStep(n) {
  ...
  g('req-s1').style.display = n === 1 ? '' : 'none';
  g('req-s2').style.display = n === 2 ? '' : 'none';
  g('req-s3').style.display = n === 3 ? '' : 'none';
...
function populateStep2() {
  ...
  if (currentUser && g('rf-name')) {
    g('rf-name').value = (currentUser.first_name + ' ' + currentUser.last_name).trim();
  }
  if (currentUser && g('rf-phone') && currentUser.phone) {
    g('rf-phone').value = currentUser.phone;
  }
  if (currentUser && g('rf-addr') && currentUser.address) {
    g('rf-addr').value = currentUser.address;
  }
...
function populateStep3() {
  ...
  g('pay-free-msg').style.display       = isFree ? '' : 'none';
  g('pay-method-section').style.display = isFree ? 'none' : '';
  g('pay-qr-section').style.display     = 'none';

  // Reset
  selectedPay = isFree ? 'free' : null;
  document.querySelectorAll('.pay-card').forEach(c => c.classList.remove('sel'));
  if (g('pay-ref')) g('pay-ref').value = '';
}
...
function selectPay(method) {
  selectedPay = method;
  document.querySelectorAll('.pay-card').forEach(c => c.classList.remove('sel'));
  g('pc-' + method).classList.add('sel');

  const fee = parseFloat(selectedDoc?.fee || 0);
  g('pay-amount-text').textContent = '₱' + fee.toFixed(2);
  g('pay-qr-section').style.display = '';
  if (g('pay-ref')) g('pay-ref').value = '';

  if (method === 'gcash') {
    g('pay-qr-icon').innerHTML = gcashQR();
    g('pay-info-text').textContent   = 'Scan QR Code or send to the number below via GCash';
    g('pay-account-text').textContent = 'GCash Number: ' + gcashNumber;
  } else {
    g('pay-qr-icon').innerHTML = mayaQR();
    g('pay-info-text').textContent   = 'Scan QR Code or send to the number below via Maya';
    g('pay-account-text').textContent = 'Maya Number: ' + mayaNumber;
  }
}
...
async function submitRequest() {
  ...
  // Show success modal
  g('success-ref').textContent = res.reference_no;
...
function resetFormFields() {
  ...
  document.querySelectorAll('.pay-card').forEach(c => c.classList.remove('sel'));
  document.querySelectorAll('.doc-card').forEach(c => c.classList.remove('sel'));
  if (g('pay-qr-section'))     g('pay-qr-section').style.display = 'none';
  if (g('pay-free-msg'))       g('pay-free-msg').style.display = 'none';
  if (g('pay-method-section')) g('pay-method-section').style.display = 'none';
}